...
최근 연구에서는 LLM의 internal state가 모델이 알고 있는 사실과 모르는 사실을
구분하는 데 활용될 수 있고, context와 parametric knowledge가 충돌하는 상태도
latent representation에서 구분될 수 있음을 보고한다
([Ni et al., 2025](https://arxiv.org/abs/2502.11677);
[Gao et al., 2025](https://arxiv.org/abs/2510.12460)).
...
단, `C`는 **LLM의 parametric knowledge를 기준으로 한 conflict**이며 시장의
consensus surprise 자체와 동일하지 않다. 두 값이 실제로 연결되는지는 2단계의
macro announcement 실험에서 별도로 검증한다.
...
이는 여러 질문 표현에서 internal confidence의 일관성을 이용해 knowledge boundary를
보정한 연구와도 부합한다
([Ni et al., 2025](https://arxiv.org/abs/2502.11677)).
...
금융 backtest의 look-ahead bias를 줄이기 위해 가능하면 시점별 학습 데이터가
통제된 point-in-time model 또는 명확한 cutoff의 고정 checkpoint를 사용한다.
최근에는 2013–2024년의 월별 checkpoint를 구축한 point-in-time language model
연구도 공개되어 있다
([Kelly et al., 2026](https://arxiv.org/abs/2607.11889)).
...
Aligned context와 conflicting context가 latent space에서 선형적으로 구분될 수
있다는 최근 결과는 이 접근의 직접적인 근거가 된다
([Gao et al., 2025](https://arxiv.org/abs/2510.12460)).
...
특히 linear probe가 의도한 내부 과정이 아니라 source, 응답 길이 또는 task format을
학습할 수 있으므로, entity와 template뿐 아니라 source와 문서 형식도 가능한 범위에서
분리한다
([Sahoo et al., 2026](https://arxiv.org/abs/2606.02907)).
...
Scheduled macro announcement에 대한 attention과 sentiment가 주식 변동성 예측을
개선할 수 있다는 결과는 macro pilot의 금융적 타당성을 뒷받침한다
([Halousková and Lyócsa, 2025](https://arxiv.org/abs/2503.19767)).
다만 이 선행연구는 `F/C` 신호 자체를 검증한 것은 아니므로, 본 연구의 직접적인
기여는 frozen internal signal의 추가 예측력을 검증하는 데 있다.
...
Point-in-time LLM으로 생성한 뉴스 representation과 시장 변수를 결합해 변동성
예측을 개선한 최근 결과는 LLM 기반 text feature의 활용 가능성을 뒷받침한다
([Kong et al., 2025](https://arxiv.org/abs/2510.20699)). 본 연구는 일반 embedding이
아니라 사전에 동결한 knowledge retrieval/conflict signal의 incremental value를
검증한다는 점에서 차별화된다.
...
아래 문헌은 모두 2025년 이후 공개된 다저자 arXiv 연구이며, 단독저자 연구는
포함하지 않았다.

1. Ni, S. et al. (2025). **Towards Fully Exploiting LLM Internal States to Enhance Knowledge Boundary Perception.** arXiv:2502.11677.  
   Internal state와 paraphrase consistency를 이용해 known/unknown boundary를 측정할 수 있다는 근거.

2. Gao, L. et al. (2025). **Beyond Black-Box Interventions: Latent Probing for Faithful Retrieval-Augmented Generation.** arXiv:2510.12460.  
   Aligned knowledge와 conflicting knowledge가 latent space에서 선형적으로 구분될 수 있다는 근거.

3. Sahoo, S. et al. (2026). **Linear Probes Detect Task Format, Not Reasoning Mode in Language Model Hidden States.** arXiv:2606.02907.  
   Linear probe가 source와 format 차이를 학습할 수 있으므로 deconfounding이 필요하다는 근거.

4. Kelly, B. et al. (2026). **Scaling Point-in-Time Language Models.** arXiv:2607.11889.  
   금융 backtest에서 미래 정보 누출을 막는 point-in-time LLM 및 월별 checkpoint 구축의 근거.

5. Halousková, M. and Lyócsa, Š. (2025). **Forecasting U.S. Equity Market Volatility with Attention and Sentiment to the Economy.** arXiv:2503.19767.  
   Scheduled macro information이 주식 변동성 예측에 추가 정보를 제공할 수 있다는 근거.

6. Kong, Y. et al. (2025). **Fusing Narrative Semantics for Financial Volatility Forecasting.** arXiv:2510.20699.  
   Point-in-time LLM 뉴스 representation과 시장 변수를 결합한 변동성 예측의 실현 가능성을 보여주는 근거.
